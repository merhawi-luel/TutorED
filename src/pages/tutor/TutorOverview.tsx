  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className="space-y-8">
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-sm font-medium" style={{ color: colors.textPrimary }}>{vInfo.label}</span>
            {vLevel === "partial" && (
        ...
        {vLevel !== "verified" && (
          <button
            ...
            style={{
              background: vLevel === "partial" ? colors.accent : colors.accentBg,
              color: vLevel === "partial" ? "#FFFFFF" : colors.accent,
              border: vLevel === "partial" ? "none" : `1px solid ${colors.accentBorder}`,
            }}
          >
        ...
        {STAT_CARDS.map((card, i) => {
          ...
          return (
            <button
              ...
              style={{ background: colors.bgCard, border: `1px solid ${colors.borderColor}`, boxShadow: "var(--shadow-card)" }}
            >
              ...
              <div className="text-xs mt-0.5" style={{ color: colors.textMuted }}>{sub}</div>
            </button>
          );
        })}
      </div>
      ...
      <div className={`fade-up delay-600 ${inView ? "in-view" : ""}`}>
        <h2 className="text-sm font-medium mb-4" style={{ color: colors.textSecondary }}>Recent Applications</h2>
        <div className="space-y-2">
          {applications.slice(0, 3).map((app) => (
            ...
            >
              <div>
                <div className="text-sm font-medium" style={{ color: colors.textPrimary }}>{app.vacancyTitle}</div>
                <div className="text-xs" style={{ color: colors.textMuted }}>{app.organizationName}</div>
              </div>
              <StatusBadge status={app.status} />
            </div>
          ))}
          {applications.length === 0 && (
            <div className="text-sm py-8 text-center" style={{ color: colors.textMuted }}>No applications yet.</div>
          )}
        </div>
      </div>
    </div>
